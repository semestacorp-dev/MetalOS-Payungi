...
import { ShoppingBag, Wrench, Truck, CreditCard, Search, Star, CheckCircle, X, User, MapPin, Package, Bike, DollarSign, Zap, Droplet, Smartphone, Phone, AlertCircle, Info, Wallet, Loader2, Building2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 relative pb-20 md:pb-0">
      ...
      {orderModal.isOpen && orderModal.item && (
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-slate-900/80 backdrop-blur-sm p-0 md:p-4 animate-in fade-in duration-300">
              <div className="bg-white w-full max-w-md rounded-t-[2rem] md:rounded-[2rem] overflow-hidden shadow-2xl relative animate-in slide-in-from-bottom-20 duration-500">
                  <button onClick={closeOrderModal} className="absolute top-4 right-4 z-20 bg-slate-100 p-2 rounded-full hover:bg-slate-200 transition">
                      <X size={20} className="text-slate-600" />
                  </button>

                  {/* HEADER */}
                  <div className="bg-gradient-to-r from-[#003D79] to-[#005e9e] p-6 text-white relative overflow-hidden">
                      <div className="absolute right-0 top-0 opacity-10"><ShoppingBag size={100} /></div>
                      <h3 className="text-xl font-bold relative z-10 flex items-center gap-2">
                          <Building2 size={20} className="text-[#FFB81C]" /> 
                          {orderModal.step === 'SUCCESS' ? 'Transaksi Berhasil' : 'Detail Pesanan'}
                      </h3>
                      <p className="text-blue-200 text-xs mt-1 relative z-10">WargaPay powered by Livin'</p>
                  </div>
                  ...
                  <div className="p-6">
                      ...
                      {orderModal.step === 'REVIEW' && (
                          <div className="space-y-6">
                              <div className="flex gap-4">
                                  ...
                                  <div>
                                      <h4 className="font-bold text-slate-800 line-clamp-2">{orderModal.item.name}</h4>
                                      ...
                                      <p className="font-black text-orange-600 text-lg mt-2">
                              ...
                              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3">
                                  <div className="flex justify-between text-sm">
                                      ...
                                      <span className="font-bold text-blue-700">WargaPay / Livin'</span>
                                  </div>
                                  <div className="flex justify-between text-sm">
                                  ...
                                  <div className="border-t border-slate-200 pt-2 flex justify-between">
                                      ...
                                      <span className="font-black text-xl text-slate-900">Rp {orderModal.item.price.toLocaleString()}</span>
                                  </div>
                              </div>

                              <button 
                                onClick={handleConfirmPayment}
                                className="w-full bg-[#003D79] text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-900/20 hover:bg-[#002d5a] transition active:scale-95 flex items-center justify-center gap-2"
                              >
                          ...
                          <div className="flex flex-col items-center justify-center py-10 text-center">
                              <div className="relative mb-6">
                                  ...
                                  <div className="absolute top-0 right-0">
                                      <Loader2 size={24} className="text-[#FFB81C] animate-spin" />
                                  </div>
                              </div>
                      ...
                      {orderModal.step === 'SUCCESS' && (
                          <div className="text-center py-4 animate-in zoom-in duration-300">
                              <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 text-green-600 shadow-[0_0_30px_rgba(34,197,94,0.3)]">
                                  <CheckCircle size={40} />
                              </div>
                              <h3 className="text-2xl font-black text-slate-800 mb-2">Pembayaran Berhasil!</h3>
                              <p className="text-slate-500 text-sm mb-8">Pesanan Anda telah diteruskan ke merchant.</p>
                              
                              <div className="bg-[#f0f9ff] border border-blue-100 rounded-xl p-4 mb-6 text-left relative overflow-hidden">
                                  <div className="absolute top-0 left-0 w-1 h-full bg-[#003D79]"></div>
                                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">ID Transaksi</p>
                                  <p className="font-mono font-bold text-slate-800 tracking-wide">LIVIN-{Math.floor(Math.random()*1000000)}</p>
                              </div>
